...
async function sendCommand(k, shifted) {
    ...
    console.log(
        `Keydown: ${k}, sending command: ${rcmd} (shifted: ${shifted})`
    );
...
function handleDarkMode() {
    ...
    if (
        (nativeTheme.shouldUseDarkColors || alwaysUseDarkMode) &&
        !neverUseDarkMode
    ) {
...
function handleContextMenu() {
    ...
    var topChecked = JSON.parse(
        localStorage.getItem("alwaysOnTopChecked") || "false"
    );
...
async function helpMessage() {
    // It's not ideal to hardcode the hotkey. It would be better to get it from the main process.
    const hotkey =
        process.platform === "darwin" ? "Cmd+Shift+0" : "Ctrl+Shift+0";
